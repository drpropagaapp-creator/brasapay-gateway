...
function reload(extra = {}) {
    ...
    router.get('/metricas/cliques', q, { preserveState: false });
}

function exportParams() {
    const params = new URLSearchParams({ period: props.period || '7dias', type: 'clicks' });
...
function exportCsv() {
    window.location.href = `/metricas/export.csv?${exportParams().toString()}`;
}

function exportXlsx() {
    window.location.href = `/metricas/export.xlsx?${exportParams().toString()}`;
}
...
<template>
    <div :class="pageClass">
        <AuroraPageHeader title="Cliques e eventos" subtitle="Log detalhado com IP mascarado (LGPD)." />
        <div class="mt-4 space-y-4">
            <MetricsNav :tab="tab" :period="period" :date_from="date_from" :date_to="date_to" :filters="filters">
                <template #filters>
                    <input
                        v-model="search"
                        type="search"
                        ...
                        @keyup.enter="reload()"
                    >
                    <button type="button" class="rounded-lg bg-zinc-200 px-3 py-1.5 text-xs dark:bg-zinc-700" @click="reload()">Buscar</button>
