...
<template>
    <aside
        :class="[
            'kawaii-sidebar fixed left-0 top-0 z-[99999] flex h-screen flex-col',
    ...
    >
        <div class="flex h-[72px] shrink-0 items-center justify-between px-5">
            ...
            >
                ...
                <span v-else class="kawaii-fg truncate text-[15px] font-extrabold tracking-tight">
                    {{ appName() }}
                </span>
            </Link>
            <button
                v-if="isMobile"
                type="button"
                class="kawaii-icon-btn ml-1 flex h-8 w-8 shrink-0 touch-manipulation cursor-pointer select-none items-center justify-center"
        ...
        <div class="kawaii-sidebar-footer shrink-0 space-y-3 px-4 py-4">
