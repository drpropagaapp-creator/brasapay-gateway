...
<template>
    <aside
        :class="[
            'studio-sidebar fixed left-0 top-0 z-[99999] flex h-screen flex-col',
    ...
    >
        <div class="flex h-[76px] shrink-0 items-center justify-between px-6">
            <Link
                :href="homeHref"
            ...
            >
                <template v-if="hasLogoFull()">
                    <img
                        ...
                        :alt="appName()"
                        class="h-8 max-w-[180px] object-contain object-left dark:hidden"
                    />
                    <img
                        v-if="appSettings().app_logo_dark"
                        :src="appSettings().app_logo_dark"
                        :alt="appName()"
                        class="hidden h-8 max-w-[180px] object-contain object-left dark:block"
                    />
                    <img
                ...
                <span v-else class="studio-fg truncate text-[16px] font-bold tracking-tight">
                    {{ appName() }}
                </span>
            </Link>
            <button
                v-if="isMobile"
                type="button"
                class="studio-icon-btn ml-1 flex h-8 w-8 shrink-0 touch-manipulation cursor-pointer select-none items-center justify-center rounded-md"
        ...
        <div class="studio-sidebar-footer shrink-0 space-y-3 px-4 py-4">
