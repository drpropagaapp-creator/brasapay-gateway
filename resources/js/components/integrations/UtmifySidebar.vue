...
async function save() {
    ...
    try {
        ...
        if (editingIntegration.value) {
            await axios.put(
                `/integracoes/utmify/${editingIntegration.value.id}`,
                payload
            );
        } else {
            if (!payload.api_key) {
            ...
            await axios.post('/integracoes/utmify', payload);
        }
        emit('saved');
        resetForm(); // volta para a lista (igual ao Webhook)
    } catch (err) {
        errorMessage.value =
...
async function confirmRemove(integration) {
    ...
    confirmingDeleteId.value = null;
    try {
        await axios.delete(`/integracoes/utmify/${integration.id}`);
        emit('saved');
            ...
            >
                ...
                <div class="flex flex-1 flex-col overflow-y-auto p-5">
                    ...
                    <template v-if="!showingForm">
                        ...
                        <ul v-if="utmify_integrations.length" class="space-y-2">
                            <li
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="text-left">
                                ...
                                <p class="mb-2 text-left text-xs text-zinc-500 dark:text-zinc-400">
                                    {{ t('integrations.assigned_products_hint', 'Selecione os produtos para os quais esta integração enviará eventos. Deixe vazio para todos os produtos.') }}
                                </p>
                                <div
